        ...
        *ngIf="!params?.isMultiSelect"
        ...
        <mat-select
          ...
          <mat-option
            ...
            *ngFor="let c of distinctOptions"
            [value]="c?.value">
            {{ c?.label }}
          </mat-option>
        </mat-select>
...
export class FloatingFilterStatusRendererComponent
  implements AgFloatingFilterComponent
{
  ...
  get distinctOptions() {
    const arrayUniqueByKey: any[] = [
      ...new Map(
        this.options.map((item: any) => [item['value'], item])
      ).values(),
    ];
  ...
  get options() {
    if (!this.componentParent?.gridData?.optionalParams.statuses) return [];
    if (this.componentParent?.selectedShipmentMode?.length === 0)
      return this.componentParent?.gridData?.optionalParams.statuses;
    return this.componentParent?.gridData?.optionalParams.statuses.filter(
      (item: any) => {
        return (
          item.unit === this.componentParent?.selectedShipmentMode ||
          item?.value == ''
        );
      }
    );
